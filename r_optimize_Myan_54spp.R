...
N_Bio<-read.csv("Nansen_Biomass_SweptArea_54Spp.csv", header = T, stringsAsFactors = F)
...
source("functions.R")
## Load data:
params_site_specific <- read_csv("data/myanmar_model_inputs_54_TL.csv",col_names=TRUE)
...
params_default <- params_default %>%
  mutate(r_max = 1e10 * trophic_level / 4.5)
...
biomassIndicatorK <- historicOutput$simResultsAggregated %>%
...
#Here using average of 1e10*trophic_level/4.5 as starting guess:  8130452685

OptR <- optim(par=8130452685,optimizationFunction,upper=3e11,lower=1e7,method="L-BFGS-B",control=list(maxit=1000))
save(OptR,file="data/MyanOptR.Rdata")
OptR$value #189675403
